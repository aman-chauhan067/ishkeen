...
export const SettingsPage: React.FC = () => {
  ...
  return (
    <PageTransition className="pt-36 pb-24 px-8 sm:px-12 max-w-4xl relative">
      <Doodle type="circle" className="absolute top-28 right-24 w-24 h-24 text-[#5C7E9A] opacity-10" delay={0.2} />
      <p className="font-editorial text-4xl text-[#253A4A] mb-10 font-black relative z-10" style={{ letterSpacing: '-0.04em' }}>
        Account Settings
      </p>

      <div className="bg-[#FCFBF8] border border-[#253A4A]/5 rounded-3xl p-8 max-w-3xl mb-8">
        <h2 className="font-sans text-xs font-bold uppercase tracking-widest text-[#253A4A] mb-6">Profile Settings</h2>
        <form onSubmit={handleEmailChange} className="max-w-md space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-widest text-[#5C7E9A] mb-2">Email Address</label>
      ...
      <div className="bg-[#FCFBF8] border border-[#253A4A]/5 rounded-3xl p-8 max-w-3xl mb-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="font-sans text-xs font-bold uppercase tracking-widest text-[#253A4A]">Active Sessions</h2>
          <Button variant="outline" onClick={revokeOtherSessions} className="text-xs">Revoke All Other Sessions</Button>
        </div>

        {loading ? (
          <p className="text-[#5C7E9A] text-sm">Loading sessions...</p>
        ) : (
          <div className="space-y-4">
            {sessions.length === 0 && (
              <p className="text-[#5C7E9A] text-sm py-4">No active sessions found.</p>
            )}
            {sessions.map((s) => (
              <div key={s.id} className="flex items-center justify-between py-4 border-b border-[#253A4A]/5 last:border-0">
                <div>
                  <p className="text-[#253A4A] font-medium text-sm flex items-center gap-2">
                    {s.device}
                    {s.is_current && (
                  ...
                  <p className="text-[#5C7E9A] text-xs mt-1">Last Active: {new Date(s.last_active).toLocaleString()}</p>
                </div>
                {!s.is_current && (
      ...
      <div className="bg-[#FCFBF8] border border-[#253A4A]/5 rounded-3xl p-8 max-w-3xl mb-8">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-[#5C7E9A] text-sm mb-4">Sign out of your account on this device. This will end your current session and return you to the login screen.</p>
