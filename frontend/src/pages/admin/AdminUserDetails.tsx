...
export const AdminUserDetails = () => {
  ...
  if (!data) return (
    <div className="text-center mt-20 font-sans text-[#5C7E9A]">User not found.</div>
  );
  ...
  return (
    <div className="space-y-8 pb-12">
      <BlurReveal>
        ...
        <div className="flex items-start justify-between gap-6">
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 rounded-full bg-[#253A4A] flex items-center justify-center text-[#FCFBF8] text-2xl font-sans shrink-0">
              {data.user.email[0].toUpperCase()}
            </div>
            <div>
              <h1 className="font-serif text-4xl text-[#253A4A] tracking-tight mb-2">{data.user.email}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12">
        <Fade delay={0.1}>
          <div className="bg-[#FCFBF8] p-8 rounded-3xl border border-[#253A4A]/5">
            <h3 className="font-sans text-xs font-bold uppercase tracking-widest text-[#253A4A] mb-6">User Profile & Limits</h3>
            <div className="space-y-4">
              <div className="flex justify-between py-3 border-b border-[#253A4A]/5">
                <span className="font-sans text-sm text-[#5C7E9A]">User ID</span>
                <span className="font-sans text-sm font-medium text-[#253A4A] font-mono text-xs">{data.user.id}</span>
              </div>
              <div className="flex justify-between py-3 border-b border-[#253A4A]/5">
                <span className="font-sans text-sm text-[#5C7E9A]">Today's Analyses</span>
                <span className="font-sans text-sm font-medium text-[#253A4A]">{data.user.today_analyses}</span>
              </div>
              <div className="flex justify-between py-3 border-b border-[#253A4A]/5">
                <span className="font-sans text-sm text-[#5C7E9A]">Remaining Daily Limit</span>
                <span className="font-sans text-sm font-medium text-[#253A4A]">
                  {data.user.role === 'admin' ? 'Unlimited' : Math.max(0, data.user.daily_analysis_limit - data.user.today_analyses)}
                </span>
              </div>
            </div>
            
            <h3 className="font-sans text-xs font-bold uppercase tracking-widest text-[#253A4A] mt-8 mb-4">Account Controls</h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              {!data.user.is_email_verified && (
                <>
                  <Button variant="outline" className="border-[#253A4A]/20" onClick={verifyEmail} isLoading={actionLoading}>
                    Verify Email
                  </Button>
                  <Button variant="outline" className="border-[#253A4A]/20" onClick={resendVerification} isLoading={actionLoading}>
                    Resend Verification
                  </Button>
                </>
              )}
            ...
            <div className="mt-4 flex gap-2">
              <input 
                ...
                className="flex-1 bg-white border border-[#253A4A]/10 rounded-xl px-4 text-sm"
        ...
        <Fade delay={0.2}>
          <div className="bg-[#FCFBF8] p-8 rounded-3xl border border-[#253A4A]/5">
            <h3 className="font-sans text-xs font-bold uppercase tracking-widest text-[#253A4A] mb-6">Skin Profile</h3>
            {data.profile ? (
              <div className="space-y-4">
                <div className="flex justify-between py-3 border-b border-[#253A4A]/5">
                  <span className="font-sans text-sm text-[#5C7E9A]">Skin Type</span>
                  <span className="font-sans text-sm font-medium text-[#253A4A] capitalize">{data.profile.skin_type}</span>
                </div>
                <div className="flex justify-between py-3 border-b border-[#253A4A]/5">
                  <span className="font-sans text-sm text-[#5C7E9A]">Sensitivity</span>
                  <span className="font-sans text-sm font-medium text-[#253A4A] capitalize">{data.profile.sensitivity}</span>
                </div>
              </div>
            ) : (
              <p className="text-sm text-[#5C7E9A] font-sans">No profile completed yet.</p>
            )}
          </div>
        </Fade>
      </div>
      ...
      <Fade delay={0.3} className="mt-12">
        <h3 className="font-sans text-xs font-bold uppercase tracking-widest text-[#253A4A] mb-6">Analysis History</h3>
        <div className="bg-white rounded-3xl border border-[#253A4A]/10 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left font-sans text-sm">
              <thead className="bg-[#FCFBF8] text-[#5C7E9A]">
                <tr>
              ...
              <tbody className="divide-y divide-[#253A4A]/5 text-[#253A4A]">
                {data.recent_analyses.map((analysis) => (
                  <tr key={analysis.id} className="hover:bg-[#FCFBF8]/50 transition-colors">
                    ...
                    <td className="p-6 text-[#5C7E9A]">{analysis.inference_time_ms}ms</td>
                    <td className="p-6 text-right">
                      <div className="flex justify-end gap-3">
                        <button 
                          onClick={() => window.open(`/app/results/${analysis.id}`, '_blank')}
                          className="text-[#5C7E9A] hover:text-[#253A4A] font-medium"
                        >
                          Open Report
                        </button>
                        <button 
                ...
                {data.recent_analyses.length === 0 && (
                  <tr>
                    <td colSpan={4} className="p-12 text-center text-[#5C7E9A]">No analyses found for this user.</td>
                  </tr>
                )}
              </tbody>
            </table>
